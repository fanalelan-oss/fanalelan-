      ...
      <header className="sticky top-0 z-[100] w-full bg-gray-900/90 backdrop-blur-xl border-b border-white/10 h-20 sm:h-24 transition-all">
        <div className="container h-full px-4 text-right" dir="rtl">
          <div className="flex h-full items-center justify-between gap-4">
            ...
            <button aria-label="تبديل القائمة" aria-expanded={isMenuOpen} className="md:hidden text-white" onClick={() => setIsMenuOpen(!isMenuOpen)}>{isMenuOpen ? <XIcon className="h-8 w-8" /> : <MenuIcon className="h-8 w-8" />}</button>
          </div>
        </div>
      </header>
